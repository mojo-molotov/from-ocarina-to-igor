...
| `test_playwright_adapter.py`       | **Oui** (Chromium)     | Le test de bout en bout réel&nbsp;: warmup, pool, mixin, screenshotter, watcher.                              |
...
- boot et `submit` qui dépassent leur `call_timeout`&nbsp;→&nbsp;`DriverDiedError` **sans hanguer** (assertion avec plafond wall-clock)&nbsp;;
