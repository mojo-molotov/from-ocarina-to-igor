...
- **KISS as a filter**: every feature, shortcut, and DSL that was refused was refused by applying KISS. The result is 3,610 SLOC (AST count, excluding blank lines, comments and docstrings; ~8,000 raw lines in `src/`) with one runtime dependency (`python-docx`).
