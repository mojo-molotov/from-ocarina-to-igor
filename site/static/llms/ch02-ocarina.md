...
| `01-identity.md` | Technical identity: Python 3.14+, v1.1.10, single runtime dependency (`python-docx`), `mypy --strict`, MIT. |
...
| `11-opinionated/05-plugins-reports.md` | Opt-in report plugins: `pretty_print_results`, `results_to_json`, `generate_docx_proof`, `timing`, run via `run_plugins` (1 plugin → sequential; N → `ThreadPoolExecutor(max_workers=N)`). Since v1.1.9 `generate_docx_proof` also parallelizes internally per case via an opt-in `max_workers` (default 1 = verbatim sequential path). Since v1.1.10 it accepts a custom `format_date` callable (default = US layout), reserves shortened output paths atomically, and reports a `_GenerationStats` ratio; name uniqueness is case-insensitive. |
...
- **DOCX hardening + custom dates (v1.1.10)**: `generate_docx_proof(..., format_date=...)` takes a `Callable[[datetime], str]` receiving the marker's datetime already in local time and returning the full bracketed replacement (default `_default_format_date` = `[%m/%d/%Y | %Hh%M:%S.%f]`). The shortened-path fallback reserves filenames atomically via `os.open(..., O_CREAT | O_EXCL | O_WRONLY, 0o644)` (no check-then-create race) and unlinks placeholders on failure; the internal counter is now `_GenerationStats(attempted, succeeded)`, so the logger reports `Generated 1/2 DOCX` on partial failure and warns `every DOCX generation failed` (not `no test case found`) when nothing is written. Campaign/suite/case name uniqueness is enforced under `unicodedata.normalize("NFC", name).casefold()`.
...
Private helpers, internal test utilities beyond the documented families, deprecated APIs, API changes after v1.1.10. Runtime debugging (geckodriver troubleshooting, Selenium session errors) is not documented.
