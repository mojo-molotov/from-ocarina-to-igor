...
| `02-auditability.md` | "Auditable in an afternoon": ~4,060 SLOC (AST count; ~8,700 raw lines in `src/`), one runtime dependency (`python-docx`), ~2 SBOM entries, MIT. Anti-No-Code. |
...
- **"Auditable in an afternoon"**: the entire framework is ~4,060 SLOC (AST count, excluding blank lines, comments and docstrings; ~8,700 raw lines in `src/`) with one runtime dependency (`python-docx`) and ~2 SBOM entries. Any competent developer can read and understand the full source in a working day. This is a design constraint, not an accident.
